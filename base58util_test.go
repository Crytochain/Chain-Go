...
var checkLBREncodingStringTests = []struct {
	version byte
	in      string
	out     string
}{
...
func TestLBRAddress(t *testing.T) {
	fmt.Println(" LBR address Encoding/Decoding test......")

	for x, test := range checkLBREncodingStringTests {
		// test encoding
		var data string
		...
		if res := LBREncode(in_byte_array, test.version); res != test.out {
			fmt.Printf("Base58 str len: %d\n", len(res))
			t.Errorf("LBREncodeAddress test #%d failed: got %s, want: %s", x, res, test.out)
		}

		// test decoding
		res1, version, err := LBRDecode(test.out)

		res := hex.EncodeToString(res1)

		if err != nil {
			t.Errorf("LBRDecode test #%d failed with err: %v", x, err)
		} else if version != addressPrefix && version != contractPrefix {
			t.Errorf("LBRDecode test #%d failed: got version: %d want: %d", x, version, test.version)
		} else if string(res) != data {
			t.Errorf("LBRDecode test #%d failed: got: %s want: %s", x, res, test.in)
		}
	}

	// test the two decoding failure cases
	// case 1: checksum error
	_, _, err := LBRDecode("3MNQE1Y")
	if err != ErrChecksum {
		t.Error("LBRDecode test failed, expected ErrChecksum")
	}
